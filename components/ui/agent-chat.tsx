...
export function AgentChat({ userName = "Vihaan" }: { userName?: string }) {
  ...
  const runStudyBuddy = async (prompt: string) => {
    ...
    // small staged statuses to feel “alive”
...
function toArtifact(
  ...
  // fallback: render whatever we got as markdown so canvas isn't blank
...
async function buildContextWindow(prompt: string, attachments: File[], messages: StudyBuddyMessage[]) {
  ...
  for (const file of files) {
    ...
    if (text && !text.startsWith('[')) { // Don't include error messages
      fileSnippets.push(`File: ${file.name}\n${text.slice(0, 3000)}`);
    } else if (text) {
      console.warn(`Failed to read file: ${text}`);
      fileSnippets.push(text); // Include error message
    }
  }
  ...
  try {
    ...
    if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
      try {
        ...
        // Clean up whitespace but keep paragraph breaks
        const cleaned = fullText.replace(/\s+/g, " ").trim();
        const result = cleaned.slice(0, 12000); // Increased limit to 12000 chars
    ...
    return cleaned.slice(0, 12000); // Increased limit
  } catch (err) {
